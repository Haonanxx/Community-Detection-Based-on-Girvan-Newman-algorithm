...
def computeBetweenness(g, src):
    ...
    adj = g.getGraph() # get adjacent lists
    ...
    for key, val in betweenness_score.items():
        if g.getBetweenness(key):
            g.updateBetweenness(key, 0.5*val)
        else:
            g.newBetweenness(key, 0.5*val)
    return g
...
def findCommunities(orig_g, adj, m, v_lst):
    ...
    while True:
        ...
        if modularity > curr_max:
            ...
            orig_g.updateCommunities(communities)
